...
// ════════════════════════════════════════════════════════════════════
// BrowserFrame — desktop window chrome for wide product shots
// ════════════════════════════════════════════════════════════════════
function BrowserFrame({ url = "app.karta.app/dashboard", children, className = "" }) {
  return (
    <div className={`rounded-2xl border border-border bg-surface overflow-hidden ${className}`}
      style={{ boxShadow: "0 40px 80px -32px rgba(15,31,24,0.45), 0 16px 36px -20px rgba(15,31,24,0.30)" }}>
      <div className="h-10 flex items-center gap-2 px-4 bg-cream border-b border-border">
        <span className="flex gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full" style={{ background: "#E5928A" }} />
          <span className="w-2.5 h-2.5 rounded-full" style={{ background: "#E8C57E" }} />
          <span className="w-2.5 h-2.5 rounded-full" style={{ background: "#8FC0A2" }} />
        </span>
        <div className="ml-3 flex-1 max-w-[340px] h-6 rounded-md bg-surface border border-border flex items-center gap-1.5 px-2.5">
          <span className="text-primary/50"><Icon.Check w={11} /></span>
          <span className="font-mono text-[10.5px] text-muted truncate">{url}</span>
        </div>
      </div>
      {children}
    </div>
...
// ════════════════════════════════════════════════════════════════════
// DashboardPreview — a faithful mini of the real organizer dashboard.
// Sidebar + topbar + stat strip + events grid. Forest/cream/gold.
// ════════════════════════════════════════════════════════════════════
function DashboardPreview() {
  const nav = [
    { sec: "Platform", items: [["Events", "Calendar", true], ["Analytics", "Chart"], ["Templates", "Grid"]] },
    { sec: "Workspace", items: [["Brand Kit", "Palette"], ["Team", "Users"], ["Billing", "CreditCard"], ["Settings", "Gear"]] },
  ];
  const stats = [["4", "events"], ["897", "registrations"], ["$4.2M", "revenue"], ["11k", "cards shared"]];
  const events = [
    { name: "Africa Tech Festival 2026", date: "12 Mar · Lagos", reg: "247", rev: "$4.2k", status: "Live", grad: "linear-gradient(135deg,#163828,#1F4D3A 55%,#2A6A50)", dot: "#2D7A4F" },
    { name: "Pan-African Climate Summit", date: "22 Apr · Nairobi", reg: "38", rev: "—", status: "Draft", grad: "linear-gradient(160deg,#1F4D3A,#3E7E5E)", dot: "#C9A45E" },
    { name: "Global Halal Summit", date: "18 Jul · Istanbul", reg: "0", rev: "—", status: "Draft", grad: "linear-gradient(135deg,#1F4D3A,#2A6A50 45%,#C9A45E)", dot: "#C9A45E" },
  ];
  return (
    <BrowserFrame>
      <div className="flex bg-cream text-ink" style={{ height: 520 }}>
        {/* Sidebar */}
        <div className="hidden md:flex w-[208px] shrink-0 border-r border-border flex-col">
          <div className="px-3.5 pt-3.5 pb-3 border-b border-border/70 flex items-center gap-2.5">
            <span className="w-7 h-7 rounded-lg" style={{ background: "linear-gradient(135deg,#1F4D3A,#2A6A50 60%,#E8C57E)" }} />
            <div className="min-w-0">
              <div className="font-display text-[14px] font-bold text-primary leading-none">Karta</div>
              <div className="flex items-center gap-1.5 mt-1">
                <span className="text-[10px] text-ink-soft truncate">Adaeze Okafor</span>
                <span className="font-mono text-[7px] tracking-[0.1em] uppercase px-1 py-0.5 rounded bg-primary-soft text-primary font-semibold">Pro</span>
              </div>
            </div>
          </div>
          <div className="flex-1 px-2 py-2 overflow-hidden">
            {nav.map((g, gi) => (
              <div key={gi} className="mb-1.5">
                <div className="px-2.5 pt-2.5 pb-1.5 font-mono text-[8px] tracking-[0.18em] uppercase text-muted/70">{g.sec}</div>
                {g.items.map(([label, icon, active], i) => {
                  const IconC = Icon[icon] || Icon.Grid;
                  return (
                    <div key={i} className={`flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg text-[12px] mb-0.5 ${active ? "bg-primary text-cream font-medium" : "text-ink-soft"}`}>
                      <span className={active ? "text-cream" : "text-primary/70"}><IconC w={14} /></span>{label}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="p-2.5 border-t border-border/70">
            <div className="flex items-center justify-between font-mono text-[8.5px] text-muted mb-1.5"><span>3 / ∞ events</span></div>
            <div className="h-1.5 rounded-full bg-primary-soft overflow-hidden mb-2"><div className="h-full rounded-full bg-primary" style={{ width: "38%" }} /></div>
            <div className="w-full inline-flex items-center justify-center gap-1.5 py-2 rounded-lg bg-primary text-cream text-[11px] font-medium"><Icon.Sparkle w={11} style={{ color: "#E8C57E" }} /> Upgrade plan</div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {/* topbar */}
          <div className="h-12 border-b border-border flex items-center justify-between px-4 lg:px-5 shrink-0">
            <span className="text-[12.5px] font-medium text-ink">Events</span>
            <div className="flex items-center gap-2">
              <div className="hidden sm:flex items-center gap-1.5 h-7 px-2.5 rounded-lg bg-surface border border-border text-muted"><Icon.Search w={12} /><span className="text-[11px]">Search</span></div>
              <span className="w-7 h-7 grid place-items-center rounded-lg text-ink-soft"><Icon.Bell w={14} /></span>
              <span className="w-7 h-7 rounded-full grid place-items-center text-cream font-display text-[10px] font-semibold" style={{ background: "linear-gradient(135deg,#2A6A50,#C9A45E)" }}>AO</span>
            </div>
          </div>
          {/* content */}
          <div className="flex-1 overflow-hidden p-4 lg:p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="font-display text-[18px] font-semibold text-primary tracking-[-0.02em]">Events</div>
                <div className="text-[11.5px] text-muted mt-0.5">Everything you're organizing, in one place.</div>
              </div>
              <div className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-primary text-cream text-[11.5px] font-medium"><Icon.Plus w={13} /> Create event</div>
            </div>
            {/* stat strip */}
            <div className="bg-surface border border-border rounded-xl px-4 py-3 mb-4 flex items-center gap-x-5 gap-y-2 flex-wrap">
              {stats.map((s, i) => (
                <React.Fragment key={i}>
                  {i > 0 && <span className="text-border hidden sm:inline">·</span>}
                  <div className="flex items-baseline gap-1.5"><span className="font-mono text-[16px] text-primary tracking-tight">{s[0]}</span><span className="text-[11px] text-ink-soft">{s[1]}</span></div>
                </React.Fragment>
              ))}
            </div>
            {/* events grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {events.map((e, i) => (
                <div key={i} className="bg-surface border border-border rounded-xl overflow-hidden">
                  <div className="relative h-[58px]" style={{ background: e.grad }}>
                    <div aria-hidden className="absolute inset-0" style={{ background: "radial-gradient(80% 90% at 92% 8%, rgba(232,197,126,0.3), transparent 60%)" }} />
                    <span className="absolute top-2 left-2 inline-flex items-center gap-1 text-[8px] font-mono tracking-[0.1em] uppercase px-1.5 py-0.5 rounded-full bg-cream/95 text-ink-soft"><span className="w-1 h-1 rounded-full" style={{ background: e.dot }} />{e.status}</span>
                  </div>
                  <div className="p-3">
                    <div className="font-display text-[12.5px] font-semibold text-ink tracking-tight leading-snug line-clamp-1">{e.name}</div>
                    <div className="font-mono text-[10px] text-muted mt-1">{e.date}</div>
                    <div className="flex items-center gap-2.5 mt-2 pt-2 border-t border-border/70 font-mono text-[10px] text-ink-soft">
                      <span><span className="text-primary">{e.reg}</span> reg</span><span className="text-border">·</span><span><span className="text-primary">{e.rev}</span></span>
                    </div>
                  </div>
                </div>
              ))}
