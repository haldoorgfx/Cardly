...
The app code is store-ready (permission strings, deep links, domain `eventera.so` all set). What remains is listing content and the console work below — most of it manual and yours to do, but this kit gives you the exact text and steps.

**Reality check:** you can build, sign, and *submit* in a day. Apple/Google **review** then takes ~1–7 days (sometimes longer for a first submission). Plan the public announcement around the **web** launch (already live); the app becomes publicly downloadable after they approve.
...
| Bundle ID (iOS) / Application ID (Android) | `com.cre8so.eventera` |
| Version | `1.1.0` (build `4005`) |
...
**Signing is already done** — a real upload keystore (`android/eventera-release.jks`) and `android/key.properties` exist locally (both gitignored — regenerating them would produce a DIFFERENT signing identity that Play would reject on an update, so back them up somewhere safe now, before anything happens to this machine). `build.gradle.kts` already wires a real release `signingConfig` from them, falling back to debug signing only when the files are absent. A signed release APK was built and verified from this exact config (`CN=Eventera, OU=Cre8so`, not debug).

Play wants an **`.aab`** (app bundle), not an `.apk`, for the Production track:
...
In **Play Console:** create the app (Application ID `com.cre8so.eventera`) → complete the listing (copy above) → Data safety → content rating questionnaire → upload the `.aab` to Production (or Closed testing first) → roll out.

> Enable **Play App Signing** when prompted — Google manages the app signing key and you keep the upload key (the `.jks` above).

---

## 5. iOS — sign & build (App Store) — needs a Mac

iOS builds **cannot** be produced on Windows. Use a Mac (or a Mac cloud/CI like Codemagic). iOS signing has **not** been set up yet — this section is still fully ahead of you.

```bash
# On a Mac with Xcode:
# 1) Apple Developer portal → register App ID com.cre8so.eventera
...
- [x] Android: real release keystore + `key.properties` + release `signingConfig` — done, verified.
- [x] `pubspec.yaml` description reflects the real app.
- [ ] `eventera.so` is live and serving `/api/render`, `/privacy`, `/api/payments/waafipay` (app hardcodes the domain — it must resolve).
...
- **Keystore / certificate passwords** and **account creation** — these are credentials; you must generate and enter them, and back up the existing Android keystore yourself.
