...
function SettingsPage() {
  ...
  return (
    <PageShell title="Settings" subtitle="Account and workspace preferences" max="860px"
      actions={<Btn variant="primary" icon="Check" onClick={() => window.toast && window.toast("Changes saved")}>Save changes</Btn>}>
      <Tabs active={tab} onChange={setTab} tabs={[{ id: "account", label: "Account" }, { id: "security", label: "Security" }, { id: "privacy", label: "Privacy & Data" }, { id: "notifications", label: "Notifications" }]} />
    ...
    <React.Fragment>
      <Panel title="Profile" className="mb-5">
        <div className="flex items-center gap-4 mb-5">
          ...
          <Btn icon="Layout" onClick={() => window.toast && window.toast("Choose a new photo")}>Change photo</Btn>
        </div>
...
function SettingsSecurity() {
  ...
  const sessions = [
    { dev: "MacBook Pro · Chrome", loc: "Lagos, NG", ip: "41.79.x.x", when: "Active now", current: true },
    { dev: "iPhone 15 · Safari", loc: "Lagos, NG", ip: "102.89.x.x", when: "2 hours ago" },
    { dev: "Windows · Edge", loc: "Accra, GH", ip: "154.160.x.x", when: "Yesterday" },
  ];
  return (
    <React.Fragment>
      <Panel title="Password" className="mb-5">
        <div className="flex items-center justify-between gap-4">
          <div><div className="text-[13.5px] text-ink font-medium">Password</div><div className="text-[12px] text-muted mt-0.5">Last changed 3 months ago</div></div>
          <Btn icon="Lock" onClick={() => window.openModal && window.openModal({ type: "form", title: "Change password", subtitle: "Use 12+ characters with a mix of types", fields: [{ key: "current", label: "Current password", type: "password", required: true }, { key: "next", label: "New password", type: "password", required: true }, { key: "confirm", label: "Confirm new password", type: "password", required: true }], submitLabel: "Update password", submitIcon: "Lock", toast: "Password updated" })}>Change</Btn>
        </div>
      </Panel>
      <Panel title="Two-factor authentication" className="mb-5">
        <div className="flex items-center justify-between gap-4 mb-4 pb-4 border-b border-border/70">
          <div className="flex items-center gap-2.5"><span className={`w-9 h-9 rounded-lg grid place-items-center shrink-0 ${twofa ? "bg-emerald-50 text-emerald-600" : "bg-ink/5 text-muted"}`}><Icon.Shield w={17} /></span><div><div className="text-[13.5px] text-ink font-medium">Authenticator app {twofa && <span className="text-emerald-600 text-[11px] font-mono ml-1">· On</span>}</div><div className="text-[12px] text-muted mt-0.5">Required for account changes & payouts</div></div></div>
          <Toggle on={twofa} onClick={() => { setTwofa(!twofa); window.toast && window.toast(twofa ? "2FA disabled" : "2FA enabled"); }} />
        </div>
        <div className="flex items-center justify-between gap-4">
          <div><div className="text-[13.5px] text-ink font-medium">SSO / SAML</div><div className="text-[12px] text-muted mt-0.5">Single sign-on for your team <span className="font-mono text-[10px] tracking-[0.1em] uppercase bg-accent/20 text-accent-dark px-1.5 py-0.5 rounded ml-1">Studio</span></div></div>
          <Btn variant="ghost" onClick={() => window.toast && window.toast("Opening SSO setup")}>Configure</Btn>
        </div>
      </Panel>
      <Panel title="Active sessions" action={<button onClick={() => window.toast && window.toast("Signed out of all other devices")} className="text-[12.5px] text-red-600 font-medium hover:underline">Sign out all</button>} pad="p-0">
        <div className="divide-y divide-border/60">
          {sessions.map((s, i) => (
            <div key={i} className="flex items-center gap-3 px-5 py-3.5">
              <span className="w-9 h-9 rounded-lg bg-primary-soft text-primary grid place-items-center shrink-0"><Icon.Lock w={15} /></span>
              <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="text-[13px] font-medium text-ink truncate">{s.dev}</span>{s.current && <Pill tone="green" dot="#2D7A4F">This device</Pill>}</div><div className="font-mono text-[11px] text-muted mt-0.5">{s.loc} · {s.ip} · {s.when}</div></div>
              {!s.current && <button onClick={() => window.toast && window.toast("Session revoked")} className="text-muted hover:text-red-600 transition-colors text-[12px] font-medium shrink-0">Revoke</button>}
            </div>
          ))}
        </div>
      </Panel>
...
function SettingsPrivacy() {
  return (
    <React.Fragment>
      <Panel title="Your data" className="mb-5">
        <div className="grid gap-3.5">
          <div className="flex items-center justify-between gap-4">
            <div><div className="text-[13.5px] text-ink font-medium">Export your data</div><div className="text-[12px] text-muted mt-0.5">Download all events, attendees & orders as CSV/JSON</div></div>
            <Btn icon="External" onClick={() => window.toast && window.toast("Export requested — we'll email a download link")}>Request export</Btn>
          </div>
          <div className="flex items-center justify-between gap-4 pt-3.5 border-t border-border/70">
            <div><div className="text-[13.5px] text-ink font-medium">Data processing agreement</div><div className="text-[12px] text-muted mt-0.5">GDPR-compliant DPA for your records</div></div>
            <Btn variant="ghost" icon="External" onClick={() => window.toast && window.toast("Opening the DPA")}>View DPA</Btn>
          </div>
        </div>
      </Panel>
      <Panel title="Attendee privacy" className="mb-5">
        <div className="grid gap-3.5">
          {[
            ["Collect marketing consent", "Add an opt-in checkbox at registration", true],
            ["Right to be forgotten", "Let attendees request data deletion", true],
            ["Anonymize after event", "Strip personal data 90 days post-event", false],
            ["Cookie consent banner", "Show on your public event pages", true],
          ].map((r, i) => (
      ...
      <div className="bg-red-50/60 border border-red-200 rounded-2xl p-5 flex items-center justify-between gap-4">
        ...
        <button onClick={() => window.openModal && window.openModal({ type: "confirm", danger: true, title: "Delete your account?", confirmLabel: "Delete everything", confirmIcon: "Trash", body: "This permanently removes your account, all events, attendees and data. This cannot be undone.", reason: true, reasonLabel: "Tell us why (optional)", onConfirm: () => window.toast && window.toast("Account deletion scheduled", { tone: "danger", icon: "Trash" }) })} className="shrink-0 px-3.5 py-2 rounded-lg border border-red-300 text-red-700 text-[13px] font-medium hover:bg-red-100 transition-colors">Delete</button>
      </div>
    </React.Fragment>
  );
}
    ...
    <Panel title="Notifications">
      <div className="grid gap-3.5">
        {[
          ...
          ["Payouts & refunds", "Money movement on your account", true],
