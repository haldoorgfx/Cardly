              ...
              return (
                <div key={s.id} className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: i < staff.length - 1 ? '1px solid #F0EDE6' : undefined, opacity: isPending ? 0.75 : 1 }}>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {isPending ? (
                      <>
                        <span className="text-[12.5px] font-medium px-2.5 py-1 rounded-full" style={{ background: '#FEF3C7', color: '#92400E', border: '1px solid #FDE68A', fontFamily: 'Inter, system-ui, sans-serif' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {ROLES.map(r => (
            <div key={r.id} className="rounded-xl p-4" style={{ border: '1px solid #E5E0D4', background: '#FFFFFF' }}>
              ...
              <p className="text-[12px] flex items-center gap-1.5" style={{ color: '#65736B' }}><X size={14} strokeWidth={1.8} className="shrink-0" /> {r.cant}</p>
            </div>
          ))}
        </div>
      </PageShell>
