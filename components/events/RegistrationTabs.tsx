...
const TABS = [
  { key: 'registrations', label: 'Attendees',  segment: 'registrations' },
  { key: 'tickets',       label: 'Tickets',    segment: 'tickets' },
  { key: 'check-in',      label: 'Check-in',   segment: 'check-in' },
  { key: 'form',          label: 'Form',       segment: 'form' },
] as const;

export function RegistrationTabs({ eventId, eventName }: Props) {
  const pathname = usePathname();
  const segment = pathname.split('/').pop() ?? '';
  const active = TABS.find(t => t.segment === segment)?.key ?? 'registrations';

  return (
    <div className="sticky top-14 z-30 border-b bg-white" style={{ borderColor: '#E5E0D4' }}>
      <div className="max-w-[1100px] mx-auto px-6 lg:px-8 pt-3 pb-0">
