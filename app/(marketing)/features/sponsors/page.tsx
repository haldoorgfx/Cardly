      ...
      <div style={{ maxWidth: 1080, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 40, alignItems: 'end', marginBottom: 48 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 12 }}>
            {[
              { n: 'Live', label: 'lead capture at the booth' },
              { n: 'Per-sponsor', label: 'booth dashboard' },
              { n: 'ROI report', label: 'after every event' },
            ].map((s) => (
              <div key={s.label} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '14px 18px', flex: '1 1 120px' }}>
                <div style={{ fontFamily: 'var(--theme-font-display, "Plus Jakarta Sans"), sans-serif', fontSize: 22, fontWeight: 700, color: '#FAF6EE', letterSpacing: '-0.03em', lineHeight: 1 }}>{s.n}</div>
        ...
        <div style={{ background: '#1A1A1A', borderRadius: '14px 14px 0 0', overflow: 'hidden', boxShadow: '0 -4px 60px rgba(0,0,0,0.5)' }}>
          ...
          <div style={{ background: '#2A2A2A', height: 36, display: 'flex', alignItems: 'center', padding: '0 14px', gap: 6 }}>
            ...
            <div style={{ background: '#3A3A3A', borderRadius: 5, height: 22, flex: 1, maxWidth: 320, marginLeft: 12, display: 'flex', alignItems: 'center', padding: '0 10px' }}>
              <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, fontFamily: 'var(--font-sans)' }}>{(process.env.NEXT_PUBLIC_APP_URL ?? '').replace(/^https?:\/\//, '')}/sponsors/safaricom</span>
            </div>
          </div>
          ...
          <div style={{ background: '#FAF6EE' }}>
            ...
            <div style={{ background: '#FFFFFF', borderBottom: '1px solid #E5E0D4', height: 44, padding: '0 16px', display: 'flex', flexDirection: 'row' as const, alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontFamily: 'var(--font-sans)', fontSize: 14, fontWeight: 700, color: '#0F1F18' }}>Safaricom</span>
                <span style={{ background: 'rgba(232,197,126,0.2)', color: '#C9A45E', fontFamily: 'var(--font-sans)', fontSize: 10, textTransform: 'uppercase' as const, borderRadius: 4, padding: '3px 8px', letterSpacing: '0.06em' }}>GOLD SPONSOR</span>
              </div>
              <div style={{ background: '#2D7A4F', color: '#FAF6EE', fontFamily: 'var(--font-sans)', fontSize: 12, fontWeight: 600, borderRadius: 8, padding: '6px 12px' }}>47 leads ↓ Export</div>
            </div>
            ...
            <div style={{ padding: 16, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 16 }}>
              {/* Left panel */}
              <div>
                <div style={{ fontFamily: 'var(--font-sans)', fontSize: 13, fontWeight: 700, color: '#0F1F18', marginBottom: 10 }}>Lead capture</div>
                {/* Scan strip */}
                <div style={{ background: '#E8EFEB', borderRadius: 8, padding: 12, display: 'flex', flexDirection: 'row' as const, gap: 12, alignItems: 'center', marginBottom: 12 }}>
                  ...
                  <div>
                    <div style={{ fontFamily: 'var(--font-sans)', fontSize: 13, color: '#0F1F18' }}>Scan attendee QR at your booth</div>
                    <div style={{ fontFamily: 'var(--font-sans)', fontSize: 11, color: '#2D7A4F', marginTop: 4 }}>2 staff scanning now</div>
                  </div>
                </div>
                {/* Lead rows */}
                {[
              ...
              {/* Right panel */}
              <div>
                <div style={{ fontFamily: 'var(--font-sans)', fontSize: 13, fontWeight: 700, color: '#0F1F18', marginBottom: 10 }}>Sponsored session</div>
                ...
                <div style={{ background: '#E8EFEB', color: '#1F4D3A', fontFamily: 'var(--font-sans)', fontSize: 12, fontWeight: 600, borderRadius: 8, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  Export leads to CSV →
                </div>
              </div>
            </div>
    ...
    <>
      ...
      <section style={{ background: C.cream, padding: 'clamp(80px, 12vw, 140px) clamp(20px, 5vw, 80px) clamp(60px, 8vw, 100px)', textAlign: 'center' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: C.primarySoft, border: `1px solid ${C.border}`, borderRadius: 999, padding: '6px 16px', marginBottom: 28 }}>
          ...
          <span style={{ fontFamily: 'var(--font-sans)', fontSize: 13, fontWeight: 500, color: C.primary, letterSpacing: '0.02em' }}>Sponsor Tools</span>
        </div>
        <h1 style={{ fontFamily: 'var(--theme-font-display, "Plus Jakarta Sans"), sans-serif', fontSize: 'clamp(36px, 6vw, 72px)', fontWeight: 700, color: C.ink, letterSpacing: '-0.02em', lineHeight: 1.1, maxWidth: 780, margin: '0 auto 24px' }}>Sponsors that show up&#8202;—&#8202;not just logos on a banner.</h1>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: 'clamp(16px, 2vw, 20px)', color: C.inkSoft, maxWidth: 600, margin: '0 auto 40px', lineHeight: 1.65 }}>Give every sponsor a booth, a lead pipeline, and a post-event report. They come back because the ROI is real.</p>
      ...
      <section style={{ background: C.surface, padding: 'clamp(60px, 8vw, 100px) clamp(20px, 5vw, 80px)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 style={{ fontFamily: 'var(--theme-font-display, "Plus Jakarta Sans"), sans-serif', fontSize: 'clamp(26px, 4vw, 40px)', fontWeight: 700, color: C.ink, letterSpacing: '-0.02em', textAlign: 'center', marginBottom: 12 }}>Four tools that make sponsors want to come back</h2>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: 16, color: C.muted, textAlign: 'center', maxWidth: 520, margin: '0 auto 56px' }}>Built for organizers who want sponsors that renew — not sponsors that ghost you after the invoice.</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
            ...
            <div style={{ background: C.cream, border: `1px solid ${C.border}`, borderRadius: 16, padding: '32px 28px' }}>
              ...
              <h3 style={{ fontFamily: 'var(--theme-font-display, "Plus Jakarta Sans"), sans-serif', fontSize: 20, fontWeight: 700, color: C.ink, marginBottom: 10 }}>Sponsor Profiles &amp; Booths</h3>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: 15, color: C.inkSoft, lineHeight: 1.65 }}>Each sponsor gets a profile page inside the event app — logo, bio, tier badge, and a contact link. Attendees browse before they approach. First impressions happen before the booth conversation.</p>
            </div>
            ...
            <div style={{ background: C.cream, border: `1px solid ${C.border}`, borderRadius: 16, padding: '32px 28px' }}>
              ...
              <h3 style={{ fontFamily: 'var(--theme-font-display, "Plus Jakarta Sans"), sans-serif', fontSize: 20, fontWeight: 700, color: C.ink, marginBottom: 10 }}>QR Lead Retrieval</h3>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: 15, color: C.inkSoft, lineHeight: 1.65 }}>Sponsors get a unique booth QR. Attendees scan once — name, title, and contact are captured automatically from their registration. Sponsors export a clean CSV of every lead after the event.</p>
            </div>
            ...
            <div style={{ background: C.cream, border: `1px solid ${C.border}`, borderRadius: 16, padding: '32px 28px' }}>
              ...
              <h3 style={{ fontFamily: 'var(--theme-font-display, "Plus Jakarta Sans"), sans-serif', fontSize: 20, fontWeight: 700, color: C.ink, marginBottom: 10 }}>Sponsored Sessions</h3>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: 15, color: C.inkSoft, lineHeight: 1.65 }}>Mark any session as sponsored and display the sponsor logo alongside it in the programme. Session Q&amp;A and attendance data feed directly into the sponsor analytics report.</p>
            </div>
            ...
            <div style={{ background: C.cream, border: `1px solid ${C.border}`, borderRadius: 16, padding: '32px 28px' }}>
              ...
              <h3 style={{ fontFamily: 'var(--theme-font-display, "Plus Jakarta Sans"), sans-serif', fontSize: 20, fontWeight: 700, color: C.ink, marginBottom: 10 }}>Sponsor Analytics</h3>
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: 15, color: C.inkSoft, lineHeight: 1.65 }}>Every sponsor gets a post-event PDF report: total leads, booth scans, session views, and engagement percentile vs. other sponsors. The data that makes renewals easy to justify.</p>
            </div>
      ...
      <section style={{ background: 'linear-gradient(135deg, #1F4D3A 0%, #2A6A50 60%, #E8C57E 100%)', padding: 'clamp(72px, 10vw, 120px) clamp(20px, 5vw, 80px)', textAlign: 'center' }}>
        <h2 style={{ fontFamily: 'var(--theme-font-display, "Plus Jakarta Sans"), sans-serif', fontSize: 'clamp(28px, 5vw, 52px)', fontWeight: 700, color: '#FFFFFF', letterSpacing: '-0.02em', maxWidth: 600, margin: '0 auto 20px', lineHeight: 1.15 }}>Sponsors that see the ROI come back. Every time.</h2>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: 17, color: 'rgba(255,255,255,0.8)', maxWidth: 480, margin: '0 auto 40px', lineHeight: 1.65 }}>Set up your sponsor suite before the event. Your sponsors will thank you with renewals.</p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
