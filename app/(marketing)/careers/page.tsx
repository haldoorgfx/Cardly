    ...
    <>
      ...
      <section style={{ background: C.cream, backgroundImage: `radial-gradient(ellipse 80% 60% at 50% -10%, ${C.primarySoft} 0%, transparent 70%)`, padding: 'clamp(80px, 12vw, 140px) clamp(20px, 5vw, 80px) clamp(60px, 8vw, 100px)', textAlign: 'center' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: C.primarySoft, border: `1px solid ${C.border}`, borderRadius: 999, padding: '6px 16px', marginBottom: 28 }}>
          ...
          <span style={{ fontFamily: 'var(--font-sans)', fontSize: 13, fontWeight: 500, color: C.primary, letterSpacing: '0.02em' }}>We&apos;re hiring</span>
        </div>
        <h1 className="font-title" style={{ fontSize: 'clamp(36px, 6vw, 72px)', fontWeight: 700, color: C.ink, letterSpacing: '-0.02em', lineHeight: 1.1, maxWidth: 720, margin: '0 auto 24px' }}>Help us build the future of events.</h1>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: 'clamp(16px, 2vw, 20px)', color: C.inkSoft, maxWidth: 600, margin: '0 auto 40px', lineHeight: 1.65 }}>Eventera is a small, focused team building the event platform we always wished existed. If you care about craft, love shipping, and want to work on something that matters to real people — keep reading.</p>
        <a href="#roles" style={{ fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 15, background: C.primary, color: C.surface, padding: '14px 28px', borderRadius: 999, textDecoration: 'none', display: 'inline-block' }}>See open roles</a>
      </section>

      {/* Why Eventera */}
      <section style={{ background: C.surface, padding: 'clamp(60px, 8vw, 100px) clamp(20px, 5vw, 80px)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 className="font-title" style={{ fontSize: 'clamp(26px, 4vw, 40px)', fontWeight: 700, color: C.ink, letterSpacing: '-0.02em', textAlign: 'center', marginBottom: 56 }}>Why Eventera</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
            {whyEventera.map((w) => (
              <div key={w.title} style={{ background: C.cream, border: `1px solid ${C.border}`, borderRadius: 16, padding: '32px 28px' }}>
                <h3 className="font-title" style={{ fontSize: 20, fontWeight: 700, color: C.ink, marginBottom: 12 }}>{w.title}</h3>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: 15, color: C.inkSoft, lineHeight: 1.65 }}>{w.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="roles" style={{ scrollMarginTop: 80, background: C.cream, padding: 'clamp(60px, 8vw, 100px) clamp(20px, 5vw, 80px)' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          <h2 className="font-title" style={{ fontSize: 'clamp(26px, 4vw, 40px)', fontWeight: 700, color: C.ink, letterSpacing: '-0.02em', textAlign: 'center', marginBottom: 12 }}>Open roles</h2>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: 16, color: C.inkSoft, textAlign: 'center', maxWidth: 480, margin: '0 auto 56px' }}>We&apos;re hiring across engineering, design, and delivery. Remote-first, async by default. If one sounds like you, apply.</p>
            ...
            {roles.map((r) => (
              <div key={r.title} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: '32px 28px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16, marginBottom: 16 }}>
                  <div>
                    <h3 className="font-title" style={{ fontSize: 20, fontWeight: 700, color: C.ink, marginBottom: 8 }}>{r.title}</h3>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                      <span style={{ fontFamily: 'var(--font-sans)', fontSize: 13, color: C.muted }}>{r.type}</span>
                      <span aria-label="Competitive compensation" title="Competitive — let's talk" className="font-title" style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.06em', color: C.accentDark, background: 'rgba(232,197,126,0.16)', border: `1px solid ${C.border}`, borderRadius: 999, padding: '2px 10px' }}>{r.comp}</span>
                    </div>
                  </div>
                  <a href={`mailto:${CAREERS_EMAIL}?subject=${encodeURIComponent(`Application · ${r.title}`)}`} style={{ fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 14, background: C.primary, color: C.surface, padding: '10px 20px', borderRadius: 999, textDecoration: 'none', display: 'inline-block', flexShrink: 0 }}>Apply</a>
                </div>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: 15, color: C.inkSoft, lineHeight: 1.65, marginBottom: 20 }}>{r.desc}</p>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {r.skills.map((sk) => (
                    <span key={sk} style={{ fontFamily: 'var(--font-sans)', fontSize: 12, fontWeight: 500, color: C.primary, background: C.primarySoft, border: `1px solid ${C.border}`, borderRadius: 999, padding: '4px 12px' }}>{sk}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      <section style={{ background: C.surface, padding: 'clamp(60px, 8vw, 100px) clamp(20px, 5vw, 80px)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <h2 className="font-title" style={{ fontSize: 'clamp(26px, 4vw, 40px)', fontWeight: 700, color: C.ink, letterSpacing: '-0.02em', textAlign: 'center', marginBottom: 56 }}>How we work</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
            {culture.map((c) => (
              <div key={c.title} style={{ borderLeft: `3px solid ${C.accent}`, paddingLeft: 24 }}>
                <h3 className="font-title" style={{ fontSize: 18, fontWeight: 700, color: C.ink, marginBottom: 10 }}>{c.title}</h3>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: 15, color: C.inkSoft, lineHeight: 1.65 }}>{c.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: 'linear-gradient(135deg, #1F4D3A 0%, #2A6A50 60%, #E8C57E 100%)', padding: 'clamp(72px, 10vw, 120px) clamp(20px, 5vw, 80px)', textAlign: 'center' }}>
        <h2 className="font-title" style={{ fontSize: 'clamp(28px, 5vw, 52px)', fontWeight: 700, color: '#FFFFFF', letterSpacing: '-0.02em', maxWidth: 600, margin: '0 auto 20px', lineHeight: 1.15 }}>Don&apos;t see your role? Send us your story anyway.</h2>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: 17, color: 'rgba(255,255,255,0.8)', maxWidth: 480, margin: '0 auto 40px', lineHeight: 1.65 }}>If you care about what we are building and have something to contribute, email us at {CAREERS_EMAIL} — tell us what you do and what you want to work on.</p>
        <a href={`mailto:${CAREERS_EMAIL}?subject=${encodeURIComponent('Working at Eventera')}`} style={{ fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 15, background: '#FFFFFF', color: C.primary, padding: '14px 28px', borderRadius: 999, textDecoration: 'none', display: 'inline-block' }}>Email {CAREERS_EMAIL}</a>
      </section>
    </>
  );
}
