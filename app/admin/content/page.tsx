...
function StatusBadge({ status }: { status: 'draft' | 'published' }) {
  return status === 'published' ? (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-mono bg-emerald-50 text-emerald-700 border border-emerald-200">
      <Globe size={9} strokeWidth={2.5} /> Published
    </span>
  ) : (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-mono bg-amber-50 text-amber-700 border border-amber-200">
    ...
    <div className="p-6 lg:p-10 max-w-[1000px]">
      ...
      {pages.length === 0 ? (
        <div className="border-2 border-dashed border-[#E5E0D4] rounded-2xl p-16 text-center">
          ...
          <code className="mt-4 block text-[12px] font-mono bg-[#FAF6EE] border border-[#E5E0D4] rounded-lg px-4 py-2 text-[#3A4A42] max-w-sm mx-auto">
        ...
        <div className="border border-[#E5E0D4] rounded-2xl overflow-hidden bg-white">
          <div className="grid grid-cols-[1fr_auto_auto_auto] px-5 py-3 bg-[#FAF6EE] border-b border-[#E5E0D4]">
            <span className="text-[11px] font-mono uppercase tracking-[0.14em] text-[#6B7A72]">Page</span>
            <span className="text-[11px] font-mono uppercase tracking-[0.14em] text-[#6B7A72] text-right">Blocks</span>
            <span className="text-[11px] font-mono uppercase tracking-[0.14em] text-[#6B7A72] text-right px-6">Status</span>
            <span className="text-[11px] font-mono uppercase tracking-[0.14em] text-[#6B7A72] text-right">Actions</span>
          </div>
          {pages.map((page) => (
            <div key={page.id} className="grid grid-cols-[1fr_auto_auto_auto] px-5 py-4 border-b border-[#E5E0D4] last:border-b-0 hover:bg-[#FAF6EE] transition-colors items-center">
              <div className="min-w-0">
                ...
                <div className="font-mono text-[11px] text-[#6B7A72] mt-0.5">/{page.slug}</div>
              </div>
              <div className="text-[13px] text-[#6B7A72] font-mono text-right px-6">
                —
              </div>
              <div className="px-6">
