    ...
    return (
      <div className="space-y-2">
        ...
        <button
          ...
          className="text-[11px] font-mono text-[#1F4D3A] hover:text-[#163828] transition-colors flex items-center gap-1"
        >
  ...
  return (
    <div className="space-y-2">
      {Object.entries(value).map(([k, v]) => (
        <div key={k}>
          <div className="text-[10px] font-mono uppercase tracking-[0.12em] text-[#6B7A72] mb-1">{k}</div>
          <FieldEditor
            name={k}
            value={v}
    ...
    <div className="border border-[#E5E0D4] rounded-xl overflow-hidden bg-white">
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="w-6 h-6 rounded-md bg-[#FAF6EE] border border-[#E5E0D4] grid place-items-center shrink-0">
          <span className="font-mono text-[9px] text-[#6B7A72]">{index + 1}</span>
        </div>
        <div className="flex-1 min-w-0">
          <div className="font-medium text-[13px] text-[#0F1F18]">
            {BLOCK_LABELS[block.type as BlockType] ?? block.type}
          </div>
          <div className="font-mono text-[10px] text-[#6B7A72] truncate">
        ...
        <div className="border-t border-[#E5E0D4] px-4 py-4 bg-[#FAF6EE]">
          <div className="space-y-4">
            {Object.entries(content).map(([key, val]) => (
              <div key={key}>
                <div className="text-[11px] font-mono uppercase tracking-[0.12em] text-[#6B7A72] mb-1.5">{key}</div>
                <FieldEditor
                  name={key}
                  value={val}
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-[#E5E0D4]">
        <div className="flex items-center gap-3 px-5 py-3 max-w-[900px] mx-auto">
          ...
          <div className="flex items-center gap-2 shrink-0 ml-auto">
            {/* Status badge */}
            <span className={`font-mono text-[10px] tracking-[0.14em] uppercase px-2 py-1 rounded-full ${
              status === 'published'
      ...
      <div className="max-w-[900px] mx-auto px-5 py-8">
        ...
        <div className="mb-6 p-4 bg-white border border-[#E5E0D4] rounded-xl">
          <div className="flex items-center justify-between">
            <div>
              <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#6B7A72] mb-1">Slug</div>
              <div className="font-mono text-[13px] text-[#3A4A42]">/{page.slug}</div>
            </div>
            <div>
              <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#6B7A72] mb-1">Blocks</div>
              <div className="font-mono text-[13px] text-[#3A4A42]">{blocks.length}</div>
            </div>
            <button
              onClick={handleDelete}
              className="text-[12px] text-[#B8423C] hover:text-[#8B2E2A] transition-colors font-mono"
            >
              Delete page
            </button>
          </div>
            ...
            <div className="absolute top-12 left-0 right-0 z-50 bg-white border border-[#E5E0D4] rounded-2xl shadow-[0_8px_32px_rgba(15,31,24,0.12)] p-3 grid grid-cols-3 gap-1.5">
              {BLOCK_TYPES.map(type => (
                ...
                >
                  ...
                  <div className="font-mono text-[10px] text-[#6B7A72]">{type}</div>
                </button>
              ))}
              <button
