      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <form onSubmit={handleSearch} className="flex items-center gap-2 flex-1 min-w-[220px]">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#65736B]" size={14} />
            <input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search by filename or alt text…"
              className="w-full pl-9 pr-4 h-9 rounded-lg border border-[#E5E0D4] bg-white text-[14px] text-[#0F1F18] placeholder:text-[#65736B] focus:outline-none focus:ring-2 focus:ring-[#1F4D3A]/20 focus:border-[#1F4D3A]"
            />
          </div>
          {search && (
            <button
              ...
              className="p-2 rounded-lg hover:bg-[#E8EFEB] text-[#65736B]"
            >
              <X size={14} />
            </button>
          )}
      ...
      {items.length === 0 ? (
        ...
        <>
          ...
          <div className="mb-4 flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer text-[13px] text-[#65736B]">
              <input
                type="checkbox"
            ...
            <div className="p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="text-[12px] text-[#65736B] mt-0.5">
                    {selected.width && selected.height
                      ? `${selected.width} × ${selected.height}px · `
                      : ''}
                    {formatBytes(selected.size_bytes)} · {selected.mime ?? '—'}
                  </p>
                  <p className="text-[12px] text-[#65736B]">
                    Uploaded {formatDate(selected.created_at)}
                  </p>
                </div>
                <button
                  onClick={() => setSelected(null)}
                  className="p-1.5 rounded-lg hover:bg-[#FAF6EE] text-[#65736B] shrink-0"
                >
                  <X size={16} />
                </button>
              </div>
              ...
              {/* URL copy */}
              <div className="mt-4">
                <label className="block text-[12.5px] uppercase tracking-[0.12em] text-[#65736B] mb-1">
                  URL
                </label>
                ...
                {editingId === selected.id ? (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      autoFocus
                      className="flex-1 h-10 px-3 rounded-lg border border-[#1F4D3A] text-[13px] text-[#0F1F18] focus:outline-none focus:ring-2 focus:ring-[#1F4D3A]/20"
                    />
                    <button
                      onClick={() => saveAlt(selected.id)}
                      disabled={savingAlt}
                      className="h-10 w-10 rounded-lg bg-[#1F4D3A] text-white flex items-center justify-center hover:bg-[#163828] disabled:opacity-60"
                    >
                      {savingAlt ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />}
                    </button>
                    <button
                      onClick={() => setEditingId(null)}
                      className="h-10 w-10 rounded-lg border border-[#E5E0D4] text-[#65736B] flex items-center justify-center hover:bg-[#FAF6EE]"
                    >
                      <X size={13} />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="flex-1 text-[13px] text-[#3A4A42] min-h-[40px] flex items-center px-3 rounded-lg border border-[#E5E0D4] bg-[#FAF6EE]">
                      {selected.alt || <span className="text-[#65736B] italic">None</span>}
                    </span>
                    <button
      ...
      {confirmDeleteId && (
        ...
        >
          <div className="bg-white rounded-2xl shadow-lift w-full max-w-sm p-6">
            ...
            <p className="text-[14px] text-[#65736B] mb-5">
        ...
        {isEditing ? (
          ...
          >
            <input
              ...
              className="flex-1 h-9 px-2 rounded-lg text-[12.5px] bg-white text-[#0F1F18] border border-[#1F4D3A] focus:outline-none min-w-0"
            />
            <button
              onClick={onSaveAlt}
              disabled={savingAlt}
              className="h-9 w-9 rounded-lg bg-[#1F4D3A] text-white flex items-center justify-center shrink-0"
            >
              {savingAlt ? <Loader2 size={11} className="animate-spin" /> : <Check size={11} />}
            </button>
            <button
              onClick={onCancelEdit}
              className="h-9 w-9 rounded-lg bg-white/80 text-[#3A4A42] flex items-center justify-center shrink-0"
            >
              <X size={11} />
            </button>
          </div>
          ...
          <div className="flex items-center gap-1 pointer-events-auto">
            <button
              ...
              className="h-9 w-9 rounded-lg bg-white/90 text-[#3A4A42] flex items-center justify-center hover:bg-white"
              title="Edit alt text"
            >
              <Pencil size={11} />
            </button>
            <button
              ...
              className="h-9 w-9 rounded-lg bg-white/90 text-[#B8423C] flex items-center justify-center hover:bg-white"
              title="Delete"
            >
              <Trash2 size={11} />
            </button>
