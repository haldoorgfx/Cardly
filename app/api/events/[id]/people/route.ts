...
// Returns confirmed registrations for the event (for networking). The caller
// must be a confirmed attendee of this event (identified by their `reg`), and
// attendee emails are never exposed to peers.
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  const { searchParams } = new URL(req.url);
  const regId = searchParams.get('reg');

  // Identity: only a confirmed attendee of this event may browse the directory.
