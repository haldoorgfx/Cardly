...
import { Check, Download, Upload, Copy, ChevronLeft, Layers, Camera, Pencil, Sparkles } from 'lucide-react';
...
function SocialIcon({ name }: { name: string }) {
  if (name === 'WhatsApp') return <svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>;
  if (name === 'Instagram') return <svg width="18" height="18" viewBox="0 0 24 24" fill="white"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>;
  if (name === 'X') return <svg width="16" height="16" viewBox="0 0 24 24" fill="white"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>;
  if (name === 'LinkedIn') return <svg width="18" height="18" viewBox="0 0 24 24" fill="white"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>;
  return null;
}
    ...
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-50">
      {pieces.map((p, i) => (
        <div key={i} className="absolute rounded-sm"
          style={{ left: p.left, top: '-12px', background: p.color, width: p.size, height: p.size,
...
/* ── Live card preview (read-only) ────────────────────────────────────────── */
function CardPreview({
  backgroundUrl, backgroundWidth, backgroundHeight, zones,
  values, photoUrls, scale, measuredHeights,
}: {
  backgroundUrl: string; backgroundWidth: number; backgroundHeight: number;
  zones: Zone[]; values: FieldValues; photoUrls: Record<string, string>;
  scale: number; measuredHeights: Record<string, number>;
}) {
  return (
    <>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={backgroundUrl} alt="Card background" className="w-full h-full object-cover" draggable={false} />
              ...
              <div className="w-full h-full overflow-hidden" style={{ borderRadius: br }}>
                {url ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center gap-1"
                    style={{ background: 'rgba(15,31,24,0.55)', backdropFilter: 'blur(4px)' }}>
                    <Camera style={{ width: '28%', height: '28%', opacity: 0.7 }} color="white" strokeWidth={1.5} />
                    <span style={{ fontSize: `${Math.max(6, 7.5 * scale)}px`, color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>
                      Add photo
                    </span>
                  </div>
                )}
...
export default function AttendeeClient({
  ...
  const SHARE_BTNS = [
    ...
    { name: 'X',        bg: '#000', href: `https://twitter.com/intent/tweet?text=${encodeURIComponent(cardCaption)}` },
  ...
  if (screen === 'success') return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10" style={{ background: '#FAF6EE' }}>
      <Confetti />
      <Toast />
      ...
      <h2 className="font-display font-bold text-[#0F1F18] text-[28px] mb-1.5 text-center">You&apos;re all set, {firstName}!</h2>
      <p className="text-[#6B7A72] text-[14.5px] text-center max-w-[300px] mb-8">Your card is ready. Share it with your network.</p>
      {resultUrl && (
        <div className="mb-8 rounded-2xl overflow-hidden w-[150px] shadow-xl border border-[#E5E0D4]" style={{ aspectRatio: `${backgroundWidth}/${backgroundHeight}` }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      <div className="grid grid-cols-4 gap-3 mb-8">
        {SHARE_BTNS.map(s => (
          <a key={s.name} href={s.href} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-2">
            <div className="h-12 w-12 rounded-2xl flex items-center justify-center shadow-md" style={{ background: s.bg }}><SocialIcon name={s.name} /></div>
      ...
      <button onClick={() => { setScreen('result'); setDownloaded(false); }} className="text-[13px] text-[#6B7A72] hover:text-[#1F4D3A] transition">← Back to my card</button>
      <div className="mt-8 text-[11px] font-mono text-[#6B7A72]/40">Made with Cardly</div>
    </div>
  );
  ...
  if (screen === 'result' && resultUrl) return (
    <div className="min-h-screen flex flex-col items-center px-4 pt-8 pb-12" style={{ background: '#FAF6EE' }}>
      <Toast />
      <div className="w-full max-w-[400px]">
        ...
        <button onClick={handleDownload}
          className="w-full h-14 rounded-2xl font-display font-bold text-[16px] text-white mb-3 flex items-center justify-center gap-2.5 hover:opacity-90 transition"
          style={{ background: '#1F4D3A', boxShadow: '0 8px 24px rgba(31,77,58,0.28)' }}>
        ...
        <div className="grid grid-cols-4 gap-2 mb-5">
          {SHARE_BTNS.map(s => (
            <a key={s.name} href={s.href} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1.5">
              <div className="h-11 w-full rounded-xl flex items-center justify-center shadow-sm" style={{ background: s.bg }}><SocialIcon name={s.name} /></div>
  ...
  /* ─────────────────────────────────────────────────────────────────────────
     E1 — Form (single unified experience)
  ───────────────────────────────────────────────────────────────────────── */
  return (
    <div className="min-h-screen" style={{ background: '#FAF6EE' }}>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-32 md:pb-16">
        ...
        <div className="mb-7 flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="font-display font-bold text-[#0F1F18] text-[22px] sm:text-[28px] leading-tight">{eventName}</h1>
          ...
          {requiredZones.length > 0 && (
            <div className="shrink-0 flex flex-col items-end gap-1.5 mt-1">
              ...
              <div className="h-1.5 w-20 rounded-full bg-[#E5E0D4] overflow-hidden">
                <div className="h-full rounded-full transition-all duration-400" style={{ width: `${progressPct}%`, background: '#1F4D3A' }} />
              </div>
            </div>
          )}
        </div>
        ...
        {/* ── Two-column layout ───────────────────────────────────────────── */}
        <div className="flex flex-col-reverse md:grid md:grid-cols-[1fr_320px] lg:grid-cols-[1fr_360px] md:gap-10 lg:gap-12 md:items-start">

          {/* ══ LEFT: Form — the ONLY way to edit ════════════════════════ */}
          <div>
            <div className="bg-white rounded-2xl border border-[#E5E0D4] overflow-hidden shadow-sm">

              {/* Photo zones */}
              {photoZones.map((z, i) => (
                <div key={z.id} className={`p-5 ${i < photoZones.length - 1 || textZones.length > 0 ? 'border-b border-[#E5E0D4]' : ''}`}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {photoUrls[z.id] && (
                      <button onClick={() => fileInputRefs.current[z.id]?.click()}
                        className="text-[11.5px] text-[#1F4D3A] font-medium hover:underline flex items-center gap-1">
                  ...
                  {photoUrls[z.id] ? (
                    /* Photo preview */
                    <div className="relative rounded-xl overflow-hidden border border-[#E5E0D4] cursor-pointer group"
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/25 transition flex items-center justify-center opacity-0 group-hover:opacity-100">
                        <span className="bg-white/90 rounded-xl px-3 py-1.5 text-[12px] font-medium text-[#0F1F18] flex items-center gap-1.5">
                    ...
                    <div
                      ...
                      style={{
                        border: dropActive === z.id ? '2px dashed #1F4D3A' : '2px dashed #C9C3B1',
                        background: dropActive === z.id ? '#E8EFEB' : '#FAF6EE',
                        padding: '28px 20px',
                      }}
                      onClick={() => fileInputRefs.current[z.id]?.click()}
                      onDragOver={e => { e.preventDefault(); setDropActive(z.id); }}
                      onDragLeave={() => setDropActive(null)}
                      onDrop={e => {
                        e.preventDefault(); setDropActive(null);
                        const file = e.dataTransfer.files[0];
                        if (file?.type.startsWith('image/')) handlePhotoSelect(z.id, file);
                      }}
                    >
                      <div className="flex flex-col items-center gap-3">
                        <div className="h-12 w-12 rounded-xl bg-[#E8EFEB] border border-[#C9C3B1] grid place-items-center">
                          <Upload size={20} strokeWidth={1.8} color="#1F4D3A" />
                        </div>
                        <div className="text-center">
                          <div className="text-[13.5px] font-semibold text-[#0F1F18]">
                            {dropActive === z.id ? 'Drop to upload' : 'Click to upload photo'}
                          </div>
                          <div className="text-[11.5px] text-[#6B7A72] mt-0.5">
                            Drag & drop or click · JPG, PNG · up to 10 MB
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              {/* Text / custom zones */}
              {textZones.map((z, i) => (
                <div key={z.id} className={`p-5 ${i < textZones.length - 1 ? 'border-b border-[#E5E0D4]' : ''}`}>
                  ...
                  {z.type === 'custom' && z.options ? (
                    <select
                      ...
                      className="w-full h-11 px-4 rounded-xl text-[14px] outline-none appearance-none cursor-pointer transition"
                    ...
                    >
                      ...
                      {(z.options as string[]).map(opt => <option key={opt} value={opt}>{opt}</option>)}
                    </select>
                  ) : (
                    <input
                      type="text"
              ...
              {/* Error */}
              {error && (
                <div className="mx-5 mb-4 px-4 py-3 rounded-xl bg-red-50 border border-red-200 text-[13px] text-red-700">{error}</div>
              )}
              ...
              <div className="p-5 border-t border-[#E5E0D4]">
                <button
                  ...
                  style={allDone
                    ? { background: '#1F4D3A', color: 'white', boxShadow: '0 8px 24px rgba(31,77,58,0.25)' }
          ...
          <div className="mb-6 md:mb-0 md:sticky md:top-6">
            ...
            <div className="mt-2.5 flex items-center justify-center gap-1.5 text-[10.5px] font-mono text-[#6B7A72]/60">
              <span className="h-1.5 w-1.5 rounded-full bg-[#2D7A4F] animate-pulse" />
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 md:hidden"
        ...
        <div className="max-w-[420px] mx-auto px-5 pb-6 pt-2">
          ...
          <div className="text-center mt-2.5 text-[10.5px] font-mono text-[#6B7A72]/40">
            Powered by <span style={{ color: '#1F4D3A', fontWeight: 600 }}>Cardly</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ position: 'fixed', top: -9999, left: -9999, visibility: 'hidden', pointerEvents: 'none' }}>
        {zones.filter(z => z.type === 'text' || z.type === 'custom').map(z => {
          const spanW = Math.round((z.w / backgroundWidth) * previewW);
          const val   = values[z.id] || z.sample || z.placeholder || '';
          return (
            <span key={z.id} ref={el => { measureSpanRefs.current[z.id] = el; }}
              style={{ display: 'block', width: `${spanW}px`, fontFamily: z.font, fontWeight: z.weight ?? 700, fontSize: `${(z.size ?? 32) * scale}px`, lineHeight: z.lineHeight ?? 1.2, letterSpacing: z.letterSpacing ? `${z.letterSpacing * scale}px` : undefined, textTransform: z.textTransform, wordBreak: 'break-word', whiteSpace: 'pre-wrap' }}>
              {val}
            </span>
          );
        })}
