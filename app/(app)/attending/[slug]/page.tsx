...
import { Ticket, IdCard, CalendarDays, MapPin, ArrowRight, Download } from 'lucide-react';

export const metadata = { title: 'Event overview' };

export default async function AttendingOverviewPage({
  params,
}: {
  ...
  const [{ data: reg }, { data: ep }] = await Promise.all([
    admin
      .from('registrations')
      .select('attendee_name, status, qr_code_token, eventera_card_url, ticket_types(name)')
      .eq('id', registrationId)
      .single(),
    admin
      .from('event_pages')
      .select('starts_at, venue_name, city, is_online')
  ...
  const when = ep?.starts_at
    ? new Date(ep.starts_at).toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })
    : null;
