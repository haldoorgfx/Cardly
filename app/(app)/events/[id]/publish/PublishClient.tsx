    ...
    <div className="min-h-screen bg-[#fafafa] flex flex-col">
          ...
          {Array.from({ length: 40 }).map((_, i) => (
            <div
              ...
              style={{
                ...
                top: '-8px',
                background: ['#6c63ff', '#f8a4d8', '#ffd28a', '#7be0c0', '#ff6058'][i % 5],
      ...
      <header className="bg-white border-b border-[#e5e5ea]">
        <div className="max-w-[1200px] mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href={`/events/${eventId}`} className="h-9 w-9 rounded-lg hover:bg-[#fafafa] grid place-items-center text-[#0f0f1a]/60">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/dashboard" className="text-[13px] text-[#0f0f1a]/60 hover:text-[#0f0f1a] ml-2">Dashboard →</Link>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 px-6 py-12 max-w-[900px] mx-auto w-full">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex h-20 w-20 rounded-full items-center justify-center text-white mb-5 shadow-lift" style={{ background: 'linear-gradient(135deg,#6c63ff,#f8a4d8)' }}>
          ...
          <p className="text-[16px] text-[#0f0f1a]/60 mt-2">{zonesCount} zones defined · Ready for attendees</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Link share */}
          <div className="bg-white rounded-2xl border border-[#e5e5ea] p-6 shadow-soft">
            <div className="text-[11px] font-mono tracking-widest text-[#0f0f1a]/45 mb-4">SHARE LINK</div>
            <div className="flex items-center gap-2 bg-[#fafafa] rounded-xl border border-[#e5e5ea] px-3 py-3 mb-4">
          ...
          {/* QR code */}
          <div className="bg-white rounded-2xl border border-[#e5e5ea] p-6 shadow-soft">
            <div className="text-[11px] font-mono tracking-widest text-[#0f0f1a]/45 mb-4">QR CODE</div>
            <div className="flex justify-center mb-4">
              {qrDataUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={qrDataUrl} alt="QR code" width={160} height={160} className="rounded-xl border border-[#e5e5ea]" />
              ) : (
                <div className="h-40 w-40 rounded-xl bg-[#fafafa] border border-[#e5e5ea] animate-pulse" />
              )}
            </div>
            <button
              onClick={handleDownloadQR}
              disabled={!qrDataUrl}
              className="w-full py-2.5 rounded-xl text-[13px] font-medium text-[#0f0f1a]/80 border border-[#e5e5ea] hover:bg-[#fafafa] transition flex items-center justify-center gap-2"
            >
          ...
          {/* Embed snippet */}
          <div className="md:col-span-2 bg-white rounded-2xl border border-[#e5e5ea] p-6 shadow-soft">
            <div className="text-[11px] font-mono tracking-widest text-[#0f0f1a]/45 mb-3">EMBED IN YOUR SITE</div>
            <div className="bg-[#0f0f1a] rounded-xl p-4 font-mono text-[12px] text-emerald-400 overflow-x-auto">
              {embedCode}
            </div>
          </div>
          ...
          {/* Preview card */}
          {backgroundUrl && (
            <div className="bg-white rounded-2xl border border-[#e5e5ea] p-6 shadow-soft flex flex-col items-center">
              <div className="text-[11px] font-mono tracking-widest text-[#0f0f1a]/45 mb-4 self-start">PREVIEW</div>
              {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          {/* Next steps */}
          <div className="bg-white rounded-2xl border border-[#e5e5ea] p-6 shadow-soft">
            <div className="text-[11px] font-mono tracking-widest text-[#0f0f1a]/45 mb-4">WHAT HAPPENS NEXT</div>
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-start gap-3">
                  <span className="h-6 w-6 rounded-full bg-[#6c63ff]/10 text-[#6c63ff] text-[11px] font-mono font-semibold grid place-items-center shrink-0 mt-0.5">{i + 1}</span>
                  <span className="text-[13px] text-[#0f0f1a]/70">{step}</span>
                </div>
              ))}
            </div>
            <Link
              href={`/events/${eventId}`}
              className="mt-5 block text-center text-[13px] text-[#6c63ff] font-medium hover:underline"
            >
