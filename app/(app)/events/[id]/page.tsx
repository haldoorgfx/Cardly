...
export default async function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const ACTION_CARDS: OverviewCard[] = [
    ...
    { id: 'eventera-card',     label: 'Eventera Card',     iconId: 'sparkles',  desc: 'The personalized card every attendee gets', href: `/events/${slug}/eventera-card`,    badge: (event.download_count ?? 0) > 0 ? `${event.download_count} shared` : null, gold: true },
