...
export default async function SponsorsPage({ params }: Props) {
  ...
  const { data: sponsors } = await admin
    .from('sponsors')
    .select('id, company_name, tier, booth_location, website_url, invite_token, created_at')
    .eq('event_id', id)
  ...
  const enriched = (sponsors ?? []).map((s: { id: string; company_name: string; tier: string | null; booth_location: string | null; website_url: string | null; invite_token: string; created_at: string }) => ({
    ...s,
