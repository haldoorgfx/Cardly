...
export default async function FormBuilderPage({ params }: Props) {
  ...
  return (
    <div className="min-h-full" style={{ background: '#FAF6EE' }}>
      <EventManageNav eventId={params.id} eventName={event.name} active="form" />
      <div className="max-w-[800px] mx-auto px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-[14px] mt-1" style={{ color: '#6B7A72' }}>
            Add custom fields to your registration — text, dropdowns, phone, checkboxes.
          </p>
        </div>
        {/* Phase 1.4 — drag-and-drop form builder built here */}
        <div
          className="rounded-2xl flex items-center justify-center py-20 text-center"
          style={{ background: 'white', border: '1px solid #E5E0D4' }}
        >
          <div>
            <div className="font-mono text-[11px] tracking-widest uppercase mb-2" style={{ color: '#6B7A72' }}>Phase 1.4</div>
            <div className="text-[14px]" style={{ color: '#3A4A42' }}>
              Add · reorder · mark required — custom registration fields
            </div>
          </div>
        </div>
      </div>
