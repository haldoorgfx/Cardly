    ...
    <div className="mb-6 grid gap-2.5">
      {publish && (
        <div className="flex items-center justify-between gap-3 bg-accent/15 border border-accent/40 rounded-xl px-4 py-3">
          ...
          <button className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-accent text-primary-dark text-[12.5px] font-semibold hover:bg-accent-dark transition-colors shrink-0">
  ...
  return (
    <div>
      ...
      <div className="relative h-[190px]" style={{ background: event.grad }}>
        ...
        <div className="relative max-w-[1100px] mx-auto px-8 h-full flex flex-col justify-end pb-5">
          ...
          <h1 className="font-display text-[28px] sm:text-[32px] font-bold text-cream tracking-[-0.02em] leading-tight">{event.name}</h1>
...
function CreateEventFlow({ onCancel, onCreated }) {
  ...
  return (
    <div className="max-w-[720px] mx-auto px-8 py-10">
      ...
      {step === 1 ? (
        <div>
          <h1 className="font-display text-[24px] font-semibold text-primary tracking-[-0.02em] mb-1">Let's set up your event</h1>
          <p className="text-ink-soft text-[14px] mb-6">The essentials — you can change all of this later.</p>
          <div className="grid gap-4">
            <Field label="Event name" value="Africa Fintech Forum 2026" />
            <div className="grid grid-cols-2 gap-4">
              <Field label="Starts" value="14 May 2026 · 09:00" />
              <Field label="Ends" value="14 May 2026 · 18:00" />
            </div>
            <div>
              ...
              <Field label="" value={online ? "Zoom · link generated on publish" : "Eko Convention Centre, Lagos"} />
            </div>
            <div className="bg-surface border border-dashed border-primary/40 rounded-lg px-3 py-4 flex items-center gap-2.5 text-primary">
              <Icon.Layout w={17} />
              <span className="text-[13px] font-medium">Upload cover photo</span>
              <span className="text-[12px] text-muted">— optional, add later</span>
            </div>
          </div>
          <div className="mt-7 flex justify-end">
            <button onClick={() => setStep(2)} className="inline-flex items-center gap-2 px-5 py-3 rounded-lg bg-primary text-cream font-medium hover:bg-primary-dark transition-colors">
        ...
        <div>
          <h1 className="font-display text-[24px] font-semibold text-primary tracking-[-0.02em] mb-1">How do you want to start?</h1>
          ...
          <div className="grid gap-3">
            {choices.map((c) => {
              ...
              return (
                <button key={c.id} onClick={() => onCreated()} className="group flex items-center gap-4 text-left bg-surface border border-border rounded-2xl p-5 hover:border-primary/50 hover:-translate-y-0.5 transition-all">
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-8 py-8">
      <div className="bg-surface border border-border rounded-2xl p-10 text-center">
        ...
        <h1 className="font-display text-[24px] font-semibold text-primary tracking-[-0.02em]">{item.label}</h1>
