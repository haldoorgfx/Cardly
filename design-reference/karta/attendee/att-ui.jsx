...
// Phone-style frame so the attendee app reads as the real on-device experience,
// but it stays responsive (frame caps at 440px, fluid below).
function Phone({ children }) {
  return (
    <div className="min-h-screen w-full grid place-items-center py-6 px-3" style={{
      background: "radial-gradient(60% 50% at 50% 0%, rgba(31,77,58,0.10), transparent 60%), #EFE9DC",
    }}>
      <div className="w-full max-w-[440px] bg-cream rounded-[34px] border-[10px] border-[#0F1F18] overflow-hidden flex flex-col shadow-2xl" style={{ height: "min(900px, calc(100vh - 48px))" }}>
        {children}
      </div>
    </div>
  );
