// editor-d22.jsx — D2.2 · Editor with Text Zone selected.
// Shares chrome with D2.1 via window.ES.
...
(function(){
  ...
  // ─── Selected text zone on the canvas ───
  function SelectedTextZone({ scale }) {
    // Zone position in canvas coords (1080 × 1920)
    ...
    return (
      <>
        ...
        <div style={{
          ...
          <EI.text size={10} sw={2}/>
          <span>full_name · text</span>
        ...
        <div style={{
          ...
          {/* Placeholder text rendered in configured style */}
          <div style={{
        ...
        {(() => {
          ...
          return positions.map((p, i) => (
            <div key={i} style={{
              ...
              borderRadius: 2,
              zIndex: 4, cursor: p.cursor,
            }}/>
          ));
        })()}

        {/* Dimension/position readout near bottom-right handle */}
        <div style={{
          position: 'absolute',
          ...
          borderRadius: 4,
          fontFamily: 'JetBrains Mono, monospace', fontSize: 9.5,
          letterSpacing: '0.02em', whiteSpace: 'nowrap', zIndex: 4,
        }}>{Z.w} × {Z.h}</div>
      </>
    );
  }
  ...
  // ─── Inputs (specialized for properties panel) ───
  function NumberInput({ value, suffix, label, mono = true }) {
    return (
      <div style={{
  ...
  function FontSelect({ value }) {
    return (
      <div style={{
        ...
        <span style={{
          flex: 1,
          fontFamily: value === 'DM Sans' ? 'DM Sans, sans-serif'
                    : value === 'JetBrains Mono' ? 'JetBrains Mono, monospace'
                    : 'Inter, sans-serif',
  ...
  function Segmented({ options, active }) {
    return (
      <div style={{
        ...
        {options.map(o => (
          <button key={o.value} title={o.title} style={{
            height: 26, padding: '0 10px', minWidth: 32,
            background: active === o.value ? T.surface : 'transparent',
            border: active === o.value ? `1px solid ${T.border}` : '1px solid transparent',
            borderRadius: 4,
            fontFamily: 'JetBrains Mono, monospace', fontSize: 11,
            fontWeight: 600,
            color: active === o.value ? T.ink : T.muted,
            cursor: 'pointer',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <button title="Eyedropper" style={{
          ...
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
            <path d="M3 21l3-3 9-9-3-3-9 9-3 3z"/>
            <path d="M14 6l4-4 3 3-4 4"/>
          </svg>
        </button>
      </div>
    );
    ...
    return (
      <>
        ...
        <div style={{ padding: '14px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <PanelCard label="Label"><TextInput value="Full Name"/></PanelCard>
          <PanelCard label="Placeholder"><TextInput value="Your name"/></PanelCard>

          <PanelCard label="Required" trailing={<Toggle on/>}>
            <div style={{
              fontFamily: 'Inter, sans-serif', fontSize: 12, color: T.muted,
            }}>Attendees can't submit without this.</div>
          </PanelCard>
          ...
          <PanelCard label="Typography">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <SliderRow value={32} max={120} suffix="px"/>
                <FontSelect value="Bold 700"/>
              </div>
              <Segmented active="center" options={[
                { value: 'left',   title: 'Align left',   icon: <EI.alignLeft size={13}/> },
                { value: 'center', title: 'Align center', icon: <EI.alignCenter size={13}/> },
                { value: 'right',  title: 'Align right',  icon: <EI.alignRight size={13}/> },
              ]}/>
            </div>
          </PanelCard>

          <PanelCard label="Color"><ColorPicker hex="#0F1F18"/></PanelCard>

          <PanelCard label="Validation">
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{
                fontFamily: 'Inter, sans-serif', fontSize: 12, color: T.inkSoft,
                flex: 1,
              }}>Max characters</div>
              <div style={{ width: 80 }}>
                <NumberInput value="80" suffix=""/>
              </div>
            </div>
          </PanelCard>
          ...
          <PanelCard label="Position">
            <div style={{
              ...
              <NumberInput label="Y" value="1180" suffix="px"/>
              <NumberInput label="W" value="920" suffix="px"/>
              <NumberInput label="H" value="200" suffix="px"/>
            </div>
          </PanelCard>
  ...
  // ─── Layers list (with one entry) ───
  function LayersList({ selectedId = 'z_01' }) {
    const layers = [
  ...
  window.EditorD22 = function EditorD22({ width = 1280, height = 860 }) {
    return (
      <div style={{
        width, height,
        background: T.cream,
        ...
        <VariantTabs active="Attendee" counts={{ Attendee: 1 }} tabs={[{ name: 'Attendee' }]}/>
