    ...
    <div className="group bg-surface border border-border rounded-2xl overflow-hidden hover:border-primary/40 transition-colors flex flex-col">
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="mt-4 pt-3 border-t border-border/70 flex items-center gap-2">
          ...
          {event.status === "live" && (
            <button onClick={() => window.toast && window.toast("Opening check-in scanner — " + event.name)} className="ml-auto inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-primary text-[12.5px] hover:bg-primary-soft/70 transition-colors" title="Check-in scanner">
              <Icon.Scan w={15} />
            </button>
          )}
        </div>
  ...
  if (!hasEvents) {
    ...
    return (
      <div className="max-w-[840px] mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 text-center">
        ...
        <h1 className="font-title text-[30px] font-normal text-primary tracking-[-0.03em]">Create your first event</h1>
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-title text-[26px] font-semibold text-primary tracking-[-0.025em]">Events</h1>
      ...
      <div className="flex flex-wrap items-center gap-2.5 mb-9">
        ...
        <button onClick={() => onOpenEvent(events[0])} className="whitespace-nowrap inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-border text-ink-soft text-[13.5px] hover:border-primary/40 hover:text-primary transition-colors">
          <Icon.Users w={15} /> View all registrations
        </button>
        <button onClick={() => window.toast && window.toast("Opening check-in scanner")} className="whitespace-nowrap inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-border text-ink-soft text-[13.5px] hover:border-primary/40 hover:text-primary transition-colors">
